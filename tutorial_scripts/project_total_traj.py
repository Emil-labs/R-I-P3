...
def create_sim_scene(sim_time_step):   
    ...
    def rotate_joint7(q, wrist_angle):
        q = q.copy()
        q[6] += wrist_angle
    ...
    for task_idx, (cube_body, target_body, wrist_angle) in enumerate(tasks):
        ...
        q_near  = rotate_joint7(q_near, wrist_angle)
        q_lift  = rotate_joint7(q_lift, wrist_angle)

        qt_above = rotate_joint7(qt_above, wrist_angle)
        qt_near  = rotate_joint7(qt_near, wrist_angle)
        qt_lift  = rotate_joint7(qt_lift, wrist_angle)

        waypoints += [
            set_gripper(q_above, open=True),
            set_gripper(q_near, open=True),
            set_gripper(q_near, open=False),
            set_gripper(q_lift, open=False),

            set_gripper(qt_above, open=False),
            set_gripper(qt_near, open=False),
            set_gripper(qt_near, open=True),
            set_gripper(qt_lift, open=True),
        ]
